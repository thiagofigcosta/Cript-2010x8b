...
public class FunctionsTests { 
    ...
    static void q() throws HashException{
         for(int i=0;i<80;i++){
             String a=Hash.genRandom().compressHash();
             if(!Hash.checkHashIntegrity(a, new Hash(a).compressHash())) System.out.println("errrr");
         }
    
        for(int i=0;i<80;i++){
            String data="a";
            String a=Hash.genRandom().compressHash();
            if(!Hash.checkHashIntegrity(a, Hash.searchHash(Hash.includeHash(data, new Hash(a))).hash.compressHash())) System.out.println("err");
        }
    }
    
    public static void main(String[] args) throws HashException { 
        //TODO: 
        //      run nao funciona sempre (steeep order)
        //      mult/div nao funciona sempre 
        q(); 
    }     
} 
