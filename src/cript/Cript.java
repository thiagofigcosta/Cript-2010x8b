            ...
            switch(steep){
                    ...
                    if(hash.isUseAIOBG()){
                        for(int j=0;j<(int)out.length()/hash.getSize();j++){
                            if(j%hash.getMultiplesUsingAIOBG()==0){
                                ...
                                for(int l=0;l<hash.getSize();l++){
                                    Operations.replace(out, l, temp[hash.getIdxAlternativeOfByteGroup()[l]]);
                                }
                            }
                        }
                    }
                ...
                case 7:
                    ...
                    for(int t=0;t<=maxValueOfX;t++){
                        ...
                        buff2=out.substring(image,out.length()-1);
                        int max=255, min=0;
    ...
    public String decript(){
        ...
        for(int i=6;i>=0;i--){
            ...
            switch(steep){
                    ...
                    if(hash.isUseAIOBG()){
                        for(int j=0;j<(int)out.length()/hash.getSize();j++){
                            if(j%hash.getMultiplesUsingAIOBG()==0){
                                ...
                                for(int l=0;l<hash.getSize();l++){
                                    Operations.replace(out, hash.getIdxAlternativeOfByteGroup()[l], temp[l]);
                                }
                            }
                        }
                    }
                ...
                case 7:
                    //TODO: funcao dos numeros random
                    int maxValueOfX=(out.length()-hash.getLinearCoefficient())/hash.getAngularCoefficient();
                    for(int t=0;t<=maxValueOfX;t++){
                        int image=t*hash.getAngularCoefficient()+hash.getLinearCoefficient();
                        String buff1,buff2;
                        buff1=out.substring(0, image-1);
                        buff2=out.substring(image+1,out.length()-1);
                        out=buff1+buff2;
                    }
                    break;
            }
