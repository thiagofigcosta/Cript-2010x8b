...
public class Cript2010x8b { 
    ...
    private void run() throws HashException{
        ...
        for(int i=0;i<this.nCript;i++){ 
            ...
            do{
                err=false;
                try{this.data=new Cript(cdH,data,cript).run();}catch(MultDivException e){err=true;if(cdH==hash){hash.setMult(Math.max(hash.getMult()-1,1));}cdH.setMult(Math.max(cdH.getMult()-1,1));}
            }while(err);
