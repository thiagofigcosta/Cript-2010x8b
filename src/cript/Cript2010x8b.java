...
public class Cript2010x8b { 
    ...
    public static final boolean MULTICRIPTWITHRANDHASH=false; 
    ...
    private void run() throws HashException{
        ...
        for(int i=0;i<this.nCript;i++){ 
            ...
            do{
                err=false;
                try{this.data=new Cript(cdH,data,cript).run();}catch(MultDivException e){err=true;if(cdH==hash){hash.setMult(hash.getMult()-1);}cdH.setMult(cdH.getMult()-1);}
            }while(err);
