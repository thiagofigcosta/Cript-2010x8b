...
public class Cript2010x8b { 
    ...
    private boolean compare() throws HashException{ 
        String temp=data; 
        Hash cdH=hash; 
        if(hashInFile&&cript){
            ExportedHashData x=Hash.searchHash(temp);
            cdH=x.hash;
            if(Hash.checkHashIntegrity(hash.compressHash(), x.hash.compressHash())){
                System.out.println("ok");
            }else{System.out.println("deu ruim");
            
                for(int m=0;m<hash.compressHash().length();m++){
                    if(!Hash.hashIntegrityBySteeps(hash.compressHash().charAt(m), x.hash.compressHash().charAt(m), m)){
                        System.out.print(m+" ");
                    }
                }System.out.println();
                System.out.println(hash.showData());
                System.out.println(x.hash.showData());
            }
            temp=x.data;
        }
        for(int i=0;i<this.nCript;i++){ 
