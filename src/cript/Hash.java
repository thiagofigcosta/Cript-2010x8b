...
public class Hash {
    ...
    private void decriptSubHash(){
        ...
        decriptedSubHash=Operations.add(decriptedSubHash, size, -addH);//size deve ser 1?
    }
    ...
    public String compressHash(){//TODO
        String hash="";
        ...
        if(this.invertBits)
            ...
            four=(char) (four*2+1);
        if(this.switchSignal)four+=16;
        four+=this.switchTable*32;
        ...
        char sixteen=(char) (this.steepOrder[3]*2+this.steepOrder[4]*16);
        if(tmp>=1){sixteen+=1;}
        tmp=this.steepOrder[5];
        if(tmp>=4){sixteen+=128;tmp-=4;}
        char seventeen=(char) (this.steepOrder[6]*4);
        if(tmp>=2){seventeen+=1;tmp-=2;}
        if(tmp>=1){seventeen+=2;}
