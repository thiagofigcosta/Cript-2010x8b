...
public class Operations {     
    ...
    public static String div(String bytes,int size, int value){ //work with fix(size must be <=2) 
        if(value<=1||size<1||size>bytes.length()||(!Cript2010x8b.USEDIVMULT)) return bytes; 
        value=Math.abs(value);
                //if(value%2==0)value++;
            size=Operations.multAndDivFixer(size);
        ...
        for(int i=0;i<max;i++){ 
            ...
            int n=0;int MAX=151;
                ...
                if(n>=MAX){
                    System.out.println("DIV ERROR(bytes,value,size)");
